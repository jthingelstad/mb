...
class TestProfileConfig:
    def test_save_and_load_default_profile(self, tmp_path, monkeypatch):
        ...
        with patch("mb.config.CONFIG_DIR", config_dir), patch("mb.config.CONFIG_FILE", config_file):
    ...
    def test_list_profiles(self, tmp_path, monkeypatch):
        ...
        with patch("mb.config.CONFIG_DIR", config_dir), patch("mb.config.CONFIG_FILE", config_file):
            ...
            save_config(
                token="tok2", username="user2", blog="https://b2.micro.blog/", profile="test"
            )
