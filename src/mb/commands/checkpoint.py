...
def get_checkpoint(
    ...
    name: str = typer.Argument(
        ..., help="Checkpoint name: timeline, heartbeat, inbox, catchup, or another saved name"
    ),
):
...
def clear_checkpoint(
    ...
    all_checkpoints: bool = typer.Option(
        False, "--all", help="Clear every saved checkpoint for the active profile"
    ),
):
    ...
    if not name:
        output_or_exit(
            {"ok": False, "error": "Provide a checkpoint name or pass --all", "code": 400}, fmt
        )
        return
